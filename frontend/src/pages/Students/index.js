...
export default function Students() {
  ...
  function handleRemove() {
    ...
    const updatedStudents = students.filter(i => i.id !== student.id);
  ...
  async function handleSearch(e) {
    const response = students.filter(i => i.name.toLowerCase().includes(e.toLowerCase()))
    setFilterStudent(response);
  }
    ...
    <Container>
        ...
        <div className="actions">
          ...
          <Form onSubmit={handleSearch}>
            <div className="input-icon left">
              ...
              <Input name="name" placeholder="Buscar aluno" onChange={e => handleSearch(e.target.value)} />
            </div>
          </Form>
        </div>
      </div>
      ...
      <div className="content">
        <table>
          ...
          <tbody>
            {students.length ? (
              (filterStudent.length ? filterStudent : students).map(s => (
                <tr key={s.id}>
                  <td>{s.name}</td>
                  <td>{s.email}</td>
