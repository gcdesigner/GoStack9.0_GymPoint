...
class PackController {
  ...
  async delete(req, res) {
    ...
    /**
     * Check if the pack is linked in any enroll
     */
    const enroll = await Enroll.findOne({ where: { pack_id: id } });
    if (enroll) {
      return res.status(400).json({
        error:
          'Pacote vinculado a alguma matrícula. Para excluir é necessário alterar ou excluir as matrículas relacionadas',
      });
    }
